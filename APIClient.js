...
exports.APIClient = function APIClient(appKey, appSecret, loginfo) {
    ...
    this.tagDevices = function(tag, ready){
        ...
        log.info('tag name : %s \t %s device_tokens added \t %s device_tokens removed \t %s apids added \t %s apids removed', tag.name, tag.removedDeviceTokens.length, tag.addedApids.length, tag.removedApids.length)        
